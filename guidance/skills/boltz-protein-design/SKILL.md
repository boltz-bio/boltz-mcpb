...
- For antibody or nanobody design, recommend `binder: {type: boltz_curated, binder: boltz_antibody}` or `binder: {type: boltz_curated, binder: boltz_nanobody}` and ask the user to confirm they do not want custom scaffold/CDR control before building the payload. For custom scaffold control, use an untagged `binder` specification with `modality` and `entities`.
